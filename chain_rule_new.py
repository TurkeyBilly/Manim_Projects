...
class First2SecondTransition(FirstScene):
    ...
    def construct(self):
        ...
        VGroup(*[formulas[i] for i in range(3)])
        VGroup(*[formulas[i] for i in range(3, 8)]).shift(DOWN)
        VGroup(*[formulas[i] for i in range(8, 11)]).arrange(DOWN).\
            shift(DOWN).move_to(3*UP).scale(2).shift(1.5*DOWN+1*RIGHT)
        axis = Axes(
            x_range=[-1.0, 6.0, 1.0], y_range=[-1.0, 20.0, 1.0],
            height=FRAME_HEIGHT*1.3, width=FRAME_WIDTH*1.3
...
class SecondScene(First2SecondTransition):
    ...
    def construct(self, break_at_point: bool = False):
        ...
        # n60
        formulas = [
            r"h'(x)=f'(g(x))*\frac{1}{2}"
        ]
        formulas = self.str_list_to_tex_list(formulas).next_to(blackboard_3.get_left())
        self.wait(3)
...
class ThirdScene(SecondScene):
    # manimgl C:\Users\jiant\anaconda3\envs\manim\projects\GL\chain_rule_new.py ThirdScene -n16
    # manimgl C:\Users\jiant\anaconda3\envs\manim\projects\GL\chain_rule_new.py ThirdScene -n25
    def construct(self, break_at_point: bool = False):
        # This scene repeats and add on things from second scene
        # From line 863 to 995 should be skipped (Anim 16)
        def add_numbers(axis: NumberLine, x_values=None, excluding=None, font_size=24, **kwargs):
            numbers = VGroup()
            for x in x_values:
                if excluding is not None and x in excluding:
        ...
        title = Text('Part 2')
        title2 = Text('Inner is Linear', font_size=20)
        new_title = Text('Part 3').move_to(title)
        new_title2 = Text('Conclusion', font_size=20).move_to(title2)
        VGroup(title, title2).arrange(DOWN).move_to(3.2*UP + 5.3*RIGHT)
        VGroup(new_title, new_title2).arrange(DOWN).move_to(3.2*UP + 5.3*RIGHT)
        self.add(title, title2)
        ...
        # Anim 11 (-n10)
        DotOnPlane._axis = plane
        x_value_tracker = ValueTracker(1)
        x_val = x_value_tracker.get_value
        x0 = DotOnPlane(3, 0).set_color(RED).\
            add_updater(lambda m: m.become(
                DotOnPlane(x_val(), 0).set_color(RED)
        ...
        self.play(
            ShowCreation(_g:=x0.copy().clear_updaters()),
            Transform(_h:=gy0.copy().clear_updaters(), gy0.copy().clear_updaters().move_to(c2p(.5, .2)))
        )

        self.play(
            x_value_tracker.animate.set_value(2)
        )

        self.play(
            ShowCreation(_a:=x0.copy().clear_updaters()),
            Transform(_b:=gy0.copy().clear_updaters(), gy0.copy().clear_updaters().move_to(c2p(1, .2)))
        )

        self.play(
            x_value_tracker.animate.set_value(3)
        )

        self.play(
            ShowCreation(_c:=x0.copy().clear_updaters()),
            Transform(_d:=gy0.copy().clear_updaters(), gy0.copy().clear_updaters().move_to(c2p(1.5, .2)))
        )

        self.play(
            x_value_tracker.animate.set_value(4)
        )

        self.play(
            ShowCreation(_e:=x0.copy().clear_updaters()),
            Transform(_f:=gy0.copy().clear_updaters(), gy0.copy().clear_updaters().move_to(c2p(2, .2)))
        )

        points = [_a, _b, _c, _d, _e, _f, _g, _h]
        ...
        equation[1].next_to(blackboard_3.get_left())
        self.play(
            ShowCreation(blackboard_3),
            ShowCreation(equation[1])
        )
        ...
        self.wait()
        self.wait()
        blackboard_4 = blackboard_3.copy()
        self.play(
            *[Uncreate(mob)
            for mob in points],
            Uncreate(blackboard_3),
            Uncreate(equation[1]),
            self.on(f_graph, g_graph, void=False)
        )
        ...
        h = lambda x: f(g(x))
        f_graph = plane.get_graph(f).set_color(YELLOW)
        new_g_graph = plane.get_graph(g, [0.1, 10]).set_color(BLUE)
        h_graph = plane.get_graph(h, [0.1, 10]).set_color(GREEN)


        new_formulas = [
            r"g(x)=\ln(x)"
        ]
        new_formulas = self.str_list_to_tex_list(new_formulas).move_to(formulas[1])

        self.play(
            ReplacementTransform(g_graph, new_g_graph),
            ReplacementTransform(formulas[1], new_formulas[0]),
            ReplacementTransform(title, new_title),
            ReplacementTransform(title2, new_title2),
            ShowCreation(h_graph), run_time=3
        )

        g_graph = new_g_graph

        fx1 = DotOnPlane(g(4), f(g(4)), inner_color=RED, outter_color=YELLOW)
        fx2 = DotOnPlane(g(5), f(g(5)), inner_color=PURPLE_C, outter_color=YELLOW)
        fx1_label = Tex(r"f_{1}").next_to(fx1, DOWN).scale(0.5)
        fx2_label = Tex(r"f_{2}").next_to(fx2, DOWN).scale(0.5)
        gx1 = DotOnPlane(4, g(4), inner_color=RED, outter_color=BLUE)
        gx2 = DotOnPlane(5, g(5), inner_color=PURPLE_C, outter_color=BLUE)
        gx1_label = Tex(r"g_{1}").next_to(gx1, DOWN).scale(0.5)
        gx2_label = Tex(r"g_{2}").next_to(gx2, DOWN).scale(0.5)
        hx1 = DotOnPlane(4, h(4), inner_color=RED, outter_color=GREEN)
        hx2 = DotOnPlane(5, h(5), inner_color=PURPLE, outter_color=GREEN)
        hx1_label = Tex(r"h_{1}").next_to(hx1, DOWN).scale(0.5)
        hx2_label = Tex(r"h_{2}").next_to(hx2, DOWN).scale(0.5)

        self.play(
            *[ShowCreation(mob) for mob in[
                fx0, gy_to_fx, fx1, fx2, gx1, gx2, hx1, hx2, 
                fx1_label, fx2_label, gx1_label, gx2_label, hx1_label, hx2_label
                ]], run_time=3
        )

        copy_group_1 = VGroup(
            x_to_g.copy().clear_updaters(),
            gx_to_gy.copy().clear_updaters(),
            gy_to_fx.copy().clear_updaters()
        )

        x_value_tracker.set_value(5)
        ...
        self.play(ShowCreation(copy_group_1), ShowCreation(copy_group_2), run_time=3)

        tangent_f = self.get_secant_line(plane, f_graph, g(4), g(5), 0)
        tangent_g = self.get_secant_line(plane, g_graph, 4, 5, 0)
        tangent_h = self.get_secant_line(plane, h_graph, 4, 5, 0)
        
        tangent_lines = VGroup(tangent_f, tangent_g, tangent_h)
        self.play(ShowCreation(tangent_lines), run_time=3)

        csf = self.cross_center(fx1, fx2)
        csh = self.cross_center(hx1, hx2)
        csg = self.cross_center(gx1, gx2)
        cs_special = self.cross_center(DotOnPlane(g(4), g(4)), DotOnPlane(g(5), g(5)))

        dashed_line_config = dict(dash_length=0.2, stroke_width=7)
        dy_line_f = DashedLine(start=csf, end=fx2, **dashed_line_config).set_color(YELLOW)
        dy_line_h = DashedLine(start=csh, end=hx2, **dashed_line_config).set_color(YELLOW)
        dy_line_g = Line(start=csg, end=gx2, **dashed_line_config).set_color(YELLOW)
        dy_label_f = BraceLabel(dy_line_f, r"\Delta y_{f}", RIGHT)
        dy_label_f.label.set_color(YELLOW)
        dy_label_h = BraceLabel(dy_line_h, r"\Delta y_{h}", RIGHT)
        dy_label_h.label.set_color(YELLOW)
        dy_label_g = BraceLabel(dy_line_g, r"\Delta y_{g}", RIGHT, brace_scale=0.3)
        dy_label_g.label.set_color(YELLOW)
        dx_line_f = Line(start=DotOnPlane(g(4), g(4)), end=cs_special, **dashed_line_config).set_color(RED)
        dx_line_h = DashedLine(start=hx1, end=csh, **dashed_line_config).set_color(RED)
        dx_line_g = DashedLine(start=gx1, end=csg, **dashed_line_config).set_color(RED)
        dx_label_f = BraceLabel(dx_line_f, r"\Delta x_{f}", DOWN, brace_scale=0.3)
        dx_label_f.label.set_color(RED)
        dx_label_h = BraceLabel(dx_line_h, r"\Delta x_{h}", DOWN)
        dx_label_h.label.set_color(RED)
        dx_label_g = BraceLabel(dx_line_g, r"\Delta x_{g}", DOWN)
        dx_label_g.label.set_color(RED)

        all_diff = VGroup(
            dy_line_f,dy_line_g,dy_line_h,
            dy_label_f, dy_label_g, dy_label_h,
            dx_line_f, dx_line_g, dx_line_h,
            dx_label_f, dx_label_g, dx_label_g
        )
        # anim 32
        frame.save_state()
        self.play(
            *[ShowCreation(mob) for mob in all_diff],
            self.off(f_graph, g_graph, h_graph, symmetry_line, void=False),
            frame.animate.scale(1/1.5).shift(3*RIGHT)
        )
        self.off(f_graph)
        self.wait(10)
        dy_special = Line(cs_special, DotOnPlane(g(5), g(5)).pos, **dashed_line_config).set_color(YELLOW)
        self.play(
            ReplacementTransform(dy_line_g.copy(), dy_special), run_time=3
        )
        self.wait(10)
        self.play(ReplacementTransform(dx_line_h.copy(), dx_line_g.copy()), run_time=3)
        self.wait(3)
        self.play(frame.animate.restore())
        new_special = VGroup(
            Tex(r"\Delta x_{h}=\Delta x_{g}=dx"),
            Tex(r"\Delta x_{f}=\Delta y_{g}=dy")
        ).arrange(DOWN).next_to(formulas[2], DOWN).scale(0.8)
        n = new_special
        n[0][0:3].set_color(RED)
        n[0][4:7].set_color(RED)
        n[1][0:3].set_color(YELLOW)
        n[1][4:7].set_color(YELLOW)
        self.play(Write(new_special))
        self.wait(5)
        self.play(
            ShowCreation(blackboard_4.shift(9*LEFT).stretch_in_place(2, 1).shift(0.5*DOWN))
        )
        
        final_chainrule = VGroup(
            Tex(r"\frac{\Delta y_{h}}{\Delta x_{h}}=\frac{\Delta y_{f}}{\Delta x_{g}}=\frac{\Delta y_{f}}{\Delta y_{g}} \frac{\Delta y_{g}}{\Delta x_{g}}"),
            Tex(r"\frac{dh}{dx}=\frac{dh}{dg} \frac{dg}{dx}"),
            self.chainrule
        ).arrange(DOWN).scale(.65).next_to(blackboard_4.get_top(), DOWN)
        self.wait(3)
        self.play(Write(final_chainrule))

    def fade_out_all_mobjects(self, values):
        self.play(
            *[FadeOut(mob)
            for mob in filter(lambda m: isinstance(m, Mobject),values)
            ]
        )
