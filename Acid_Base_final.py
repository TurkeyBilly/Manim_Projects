...
class WeakAcidStrongBase(Scene):
    def construct(self, pKa: float = 4.74, acid_volume: float = 10.0, 
        ...
        ha_concentration_plane = NumberPlane(
            **common_plane_config, **n_conf,
            y_range=[0, y_max, y_max / 7.],
        ).scale(0.5).shift(12.*RIGHT + 7.5* UP)
        ha_concentration_plane.add_coordinate_labels()
        self.add(
            ha_concentration_plane.get_x_axis_label("mL(Base)", direction=UP),
        ...
        ).scale(0.5).shift(12.*RIGHT)
        oh_concentration_plane.add_coordinate_labels()
        self.add(
            oh_concentration_plane.get_x_axis_label("mL(Base)", direction=RIGHT),
...
# manimgl C:\Users\jiant\anaconda3\envs\manim\projects\GL\Acid_Base.py WeakBaseStrongAcid
class WeakBaseStrongAcid(Scene):
    def construct(self, pKb: float = 4.74, base_volume: float = 40.0, 
        ...
        b_conentration_plane.add(
            b_conentration_plane.get_x_axis_label("mL(Acid)", direction=RIGHT),
        ...
        self.add(
            bh_concentration_plane.get_x_axis_label("mL(Acid)", direction=UP),
...
# manimgl C:\Users\jiant\anaconda3\envs\manim\projects\GL\Acid_Base.py StrongAcidStrongBase
class StrongAcidStrongBase(Scene):
    def construct(self, vTitrated: float = 50, mTitrant: float = 1, mTitrated: float = 1,
                tTitrantIsAcid:bool = True, demo: bool = False):
        fr: CameraFrame = self.camera.frame
        fr.scale(2).shift(4*UP+6*RIGHT)

        def calc_pH_SS(titrant_volume: float, titrant_molarity: float,
                        titrated_volume: float, titrated_molarity: float, tTitrantIsAcid: bool = True
            ) -> float:
            """ 
        ...
        plane.add_coordinate_labels()
        self.add(
            plane.get_x_axis_label(f"mL{(label_type)}", direction=RIGHT),
            plane.get_y_axis_label("pH", direction=UP)
        )
        ...
        oh_concentration_plane.add_coordinate_labels()
        self.add(
            oh_concentration_plane.get_x_axis_label(f"mL{(label_type)}", direction=RIGHT),
            oh_concentration_plane.get_y_axis_label("pOH", direction=UP)
        )
        ...
        graph = plane.get_graph(
            lambda x: calc_pH_SS(x + 0.1, mTitrant, vTitrated, mTitrated, tTitrantIsAcid),
            use_smoothing=False, # step_size=.10,
            x_range=[0, titrated-1]
        )

        oh_conc_graph = oh_concentration_plane.get_graph(
            lambda x: 14 - calc_pH_SS(x + 0.1, mTitrant, vTitrated, mTitrated, tTitrantIsAcid),
            use_smoothing=False, # step_size=.10,
            x_range=[0, titrated-1]
        )
...
class FinalScene(StrongAcidStrongBase, WeakAcidStrongBase, WeakBaseStrongAcid):
    def construct(self):
        ...
        if flagStrongAcid and flagStrongBase: # TODO
            if t == "Acid":
                return StrongAcidStrongBase.construct(self, vBase, cAcid, cBase, True, True)
            else:
                return StrongAcidStrongBase.construct(self, vAcid, cAcid, cBase, False, True)
